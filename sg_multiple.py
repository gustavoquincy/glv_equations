    ...
    for x in range(result.size):
        ...
        for y in range(x,result.size):
            if np.sqrt(np.sum(np.square(np.array(result['percentage'][x])-np.array(result['percentage'][y]))))<threshold:#当2个物种得欧式距离小于阈值threshold的时候
                ...
                result['percentage'][y]=tuple(result['percentage'][y])#这两个物种不懂
    return result.drop_duplicates(subset=['percentage']).size,result.value_counts().tolist()/np.sum(result.value_counts().tolist())#返回2个值，一个是里层循环有多少种稳态情况，一个是每种稳态占比
...
def func(NumSpecies,timespan,reapt,threshold,showindex):
    ...
    for i in range(reapt[0]):#reapt[0]外层循环，意思是随机生长速度、相互作用和稀释度
        ...
        interaction=tempt*(0.4+1.3*(1-rd()**3)-0.4*rd()+2*0.4*rd()*rd(NumSpecies,NumSpecies))*(rd(NumSpecies,NumSpecies)<(1-0.2*rd()**2))#相互作用矩阵，tempt为符号，中间是相互作用矩阵，最后的符号判断(rd(NumSpecies,NumSpecies)<(1-0.2*rd()**2))是随机赋予随机个0，因为相互作用非全连接
        for index in range(NumSpecies):interaction[index,index]=-1#将相互作用矩阵对角线全赋值-1，因为自身对自身的作用为-1
        dilution=0.1+0.1*rd()#稀释度随机赋值
        if i%showindex==0:print(i,'th Loop cumcost:',int((time.time()-start_time)/60),'minutes')#选着适当输出间隔showindex
        loop=pd.DataFrame(columns=['percentage'])#用空pandas来装结果
        for j in range(reapt[1]):#里层循环reapt[1]，意思是让初始值随便选，看在这么多随机里面有多少种单稳态和多稳态出现
            initialtemp=rd(NumSpecies)#无特定意义，为初始值铺垫
            initial=initialtemp/np.sum(initialtemp)#我们的初始值initial
            loop.at[j,'percentage']=GLV(timespan,initial,NumSpecies,growth_rate,Sigma,interaction,dilution)#用微分方程解码器返回丰度值装到loop这个空得pandas里面
...
if __name__=='__main__':
    ...
    reapt=[200,500]#外层循环200次，里层循环500次
